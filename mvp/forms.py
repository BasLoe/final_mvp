...
class RequestResetForm(FlaskForm):
    email = StringField('Email',
                        validators=[DataRequired(), Email()])
    submit = SubmitField('Passwort jetzt zurücksetzen')

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user is None:
            raise ValidationError('Es existiert kein Account mit dieser E-Mailadresse. Bitte registriere dich vorher.')


class ResetPasswordForm(FlaskForm):
    password = PasswordField('Passwort', validators=[DataRequired()])
    confirm_password = PasswordField('Passwort zurücksetzen',
                                     validators=[DataRequired(), EqualTo('password')])
